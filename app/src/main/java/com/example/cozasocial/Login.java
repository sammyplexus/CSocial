...
public class Login extends Activity {
    ...
    public void logmein(View view) {
            ...
            Log.d("Got to this point", "Next thing is to check if details are filled in");
            Log.d("username has this", username);
            Log.d("password has this", password);
            if (username.length() < 4 || password.length() < 6) {
                YoYo.with(Techniques.Shake)
                        .duration(1800)
                        .playOn(findViewById(R.id.animeTest));
                Toast.makeText(getApplicationContext(), "Passwords have to be 4 characters or more, Passwords have to be 6 characters or more", Toast.LENGTH_LONG).show();
            } else {
                try {
            ...
            try {
                JsonArrayRequest jsonObject = new JsonArrayRequest(url, new Response.Listener<JSONArray>() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        Log.d("The error is ", error.toString());
                        toast1();
                    }
                }
                );
